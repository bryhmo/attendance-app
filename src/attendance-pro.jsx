    ...
    <div style={{minHeight:"100vh",background:"#0a0a0f",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{width:"100%",maxWidth:400}}>
        <div style={{textAlign:"center",marginBottom:40}}>
          <div style={{width:64,height:64,borderRadius:18,background:"linear-gradient(135deg,#00ff88,#00aacc)",display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:28,marginBottom:16,boxShadow:"0 0 40px rgba(0,255,136,.35)"}}>
            ◈
          </div>
        ...
        <div style={{background:"#0d0d1a",border:"1px solid #1e1e3a",borderRadius:20,padding:32,display:"flex",flexDirection:"column",gap:20}}>
          ...
          <div>
            ...
            <input value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&submit()} placeholder="you@school.edu"
              style={{width:"100%",padding:"12px 16px",background:"#111128",border:"1px solid #1e1e3a",borderRadius:10,color:"#d0d0e0",fontFamily:"'Space Mono',monospace",fontSize:13,outline:"none",boxSizing:"border-box"}}
          ...
          <div>
            ...
            <div style={{position:"relative"}}>
              <input type={showPass?"text":"password"} value={pass} onChange={e=>setPass(e.target.value)} onKeyDown={e=>e.key==="Enter"&&submit()} placeholder="••••••••"
                style={{width:"100%",padding:"12px 44px 12px 16px",background:"#111128",border:"1px solid #1e1e3a",borderRadius:10,color:"#d0d0e0",fontFamily:"'Space Mono',monospace",fontSize:13,outline:"none",boxSizing:"border-box"}}
                onFocus={e=>e.target.style.borderColor="#00ff88"} onBlur={e=>e.target.style.borderColor="#1e1e3a"}/>
...
function CameraScanner({ onDetected, onClose }) {
  ...
  const runBarcodeDetector = useCallback((detector) => {
    const tick = async () => {
      if (doneRef.current) return;
  ...
  useEffect(() => {
    ...
    const tryLoadScript = (src) => new Promise((res, rej) => {
    ...
    const init = async () => {
      ...
      // 2a. Try native BarcodeDetector (Chrome 83+, Edge, Android Chrome)
      ...
      // 2b. Try loading jsQR from multiple CDNs
      setStatus("Loading QR engine...");
      if (window.jsQR) {
        if (!cancelled) { setScanMethod("jsQR"); setStatus("Point camera at the QR code..."); runJsQR(); }
        return;
      }
      const cdns = [
      ...
      if (loaded && !cancelled) {
        setScanMethod("jsQR"); setStatus("Point camera at the QR code..."); runJsQR();
      } else if (!cancelled) {
        setError("QR engine unavailable. Use Manual Entry below.");
        setShowManual(true);
      }
    };
                ...
                <div style={{width:190,height:190,position:"relative"}}>
                  {[{top:0,left:0},{top:0,right:0},{bottom:0,left:0},{bottom:0,right:0}].map((pos,i)=>(
                    <div key={i} style={{position:"absolute",width:28,height:28,...pos,
                      borderTop:   ("top"    in pos)?"3px solid #00ff88":"none",
                      borderBottom:("bottom" in pos)?"3px solid #00ff88":"none",
                      borderLeft:  ("left"   in pos)?"3px solid #00ff88":"none",
                      borderRight: ("right"  in pos)?"3px solid #00ff88":"none"}}/>
                  ))}
        ...
        <div style={{padding:"14px 20px 20px"}}>
          ...
          {showManual && (
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              <div style={{fontFamily:"'Space Mono',monospace",fontSize:10,color:"#5a5a7a",letterSpacing:1}}>
                Copy the token shown under the teacher's QR code:
              </div>
              <input value={manualToken} onChange={e=>setManualToken(e.target.value)}
                onKeyDown={e=>e.key==="Enter"&&submitManual()}
                placeholder="ATT-CS301-..."
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:18}}>
      <div style={{background:"#0d0d1a",border:"1px solid #1e1e3a",borderRadius:20,padding:28,display:"flex",flexDirection:"column",alignItems:"center",gap:18}}>
        ...
        <div style={{fontFamily:"'Space Mono',monospace",fontSize:9,color:"#3a3a5a",textAlign:"center"}}>Students can copy this token if camera scan fails</div>
      </div>
        ...
        {(session.scanned||[]).length === 0
          ...
          : (session.scanned||[]).map(s=>(
            <div key={s.studentId} style={{padding:"11px 18px",borderBottom:"1px solid #111128",display:"flex",alignItems:"center",gap:12,animation:"slideIn .3s ease"}}>
              ...
              <span style={{marginLeft:"auto",color:"#00ff88",fontSize:16}}>OK</span>
            </div>
          ))
        }
      </div>
      ...
      <div style={{background:"#0d0d1a",border:`1px solid ${simming?"#00ff88":"#1e1e3a"}`,borderRadius:20,padding:28,display:"flex",flexDirection:"column",alignItems:"center",gap:18,transition:"border .3s"}}>
        ...
        <div style={{width:130,height:130,borderRadius:14,border:`2px solid ${simming?"#00ff88":"#1e1e3a"}`,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden",transition:"border .3s"}}>
          {simming
            ...
            : <div style={{textAlign:"center"}}><div style={{fontSize:38,marginBottom:6}}>CAM</div><div style={{fontFamily:"'Space Mono',monospace",fontSize:9,color:"#2a2a4a",letterSpacing:1}}>POINT AT QR</div></div>
          }
        </div>
        {scanResult && (
        ...
        <div style={{fontFamily:"'Space Mono',monospace",fontSize:9,color:"#2a2a4a",textAlign:"center",letterSpacing:1,lineHeight:1.6}}>
          Camera: real scan | Simulate: auto-detect open session
        </div>
      </div>
...
export default function App() {
  ...
  const tabs = isTeacher
    ? [{id:"main",label:"SESSIONS",icon:"T"},{id:"reports",label:"REPORTS",icon:"R"}]
    : [{id:"scan",label:"SCAN",icon:"S"},{id:"reports",label:"REPORTS",icon:"R"}];
        ...
        <div style={{width:"100%",maxWidth:520,padding:"24px 20px 0",display:"flex",flexDirection:"column",gap:16}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              <div style={{width:38,height:38,borderRadius:10,background:"linear-gradient(135deg,#00ff88,#00aacc)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,boxShadow:"0 0 18px rgba(0,255,136,.3)"}}>Q</div>
              <div>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              <button onClick={()=>setUser(null)} style={{width:32,height:32,borderRadius:8,border:"1px solid #1e1e3a",background:"#0d0d1a",color:"#5a5a7a",cursor:"pointer",fontSize:12,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Space Mono',monospace"}}>OUT</button>
            </div>
          </div>
